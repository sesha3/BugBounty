...
$(document).ready(function (e) {
    ...
    $("#attach-file").ejUploadbox({
        ...
        error: function (e) {
            if (fileExtension !== ".zip" && fileExtension !== ".rar" && fileExtension !== ".zipx" && fileExtension !== ".7z") {
                $("#attach-file-textbox").addClass("validation-failed-text").val("Invalid file format");
            }
            else {
                $("#attach-file-textbox").addClass("validation-failed-text").val("File size invalid");
            }
    ...
    $("#reply-attach-file").ejUploadbox({
        ...
        showFileDetails: false,
        buttonText: { browse: ".  .  ." },
        extensionsAllow: ".png,.jpeg,.jpg,.zip",
        height: 26,
        fileSize: 30000000,
        multipleFilesSelection: false,
        begin: function () {
        ...
        error: function (e) {
            debugger;
            if (fileExtension !== ".zip" && fileExtension !== ".rar" && fileExtension !== ".zipx" && fileExtension !== ".7z") {
                $("#reply-attach-file-textbox").addClass("validation-failed-text").val("Invalid file format");
            }
            else {
                $("#reply-attach-file-textbox").addClass("validation-failed-text").val("File size invalid");
            }
    ...
    $("#update-cancel").on("click", function () {
        ...
        $("#reply-attach-file-textbox").val("BrowsePath");
...
function renderMdeForSupport(id) {
    var simplemde = new EasyMDE({
        ...
        placeholder: "Type your message or paste images here.",
        toolbar: false,
...
function saveBugReport() {
    ...
    var bugData = {
        ...
        Platform: $("#select-option").val().trim(),
        Description: $("#updateMde").val().trim(),
        Attachments: $("#support-file-path-update").val()
    };
    $.ajax({
        type: "POST",
        url: saveBugReportUrl,
        data: { bugData: JSON.stringify(bugData) },
        success: function (result) {
            if (result) {
                window.location.href = bugListUrl;
            }
            else {
                alert("Error while update the bug");
            }
        }
    });
}
...
$(document).on("keyup", "#title", function (e) {
    $("#save-confirm").removeAttr("disabled");
});

$(document).on("keyup", "#updateMde", function (e) {
    $("#save-confirm").removeAttr("disabled");
});
